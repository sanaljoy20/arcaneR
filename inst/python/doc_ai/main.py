...
def _insert_comments(text, elements, lang):
    lines = text.split('\n')
    for element in elements:
        ...
        for line in lines:
            line_num += 1
            if element['line_of_code'] in line:
                leading_space = len(line) - len(line.lstrip())
                unqc = _unquote(element['comment']).split('\n')
                unqc = [u.strip() for u in unqc]
                if lang == "R":
                    lines[line_num] = " "*leading_space + "\n".join(unqc) + '\n' + line
                else:
                    lines[line_num] = line + '\n' + " "*leading_space + "\n".join(unqc)
                found = True
                break
        if not found:
            util_print.print_warning(f"Could not match AI element '{element['line_of_code']}' to a line of the file")
    return '\n'.join(lines)
...
def comment_file(path_to_src_file, options):
    ...
    if options.out_dir is None:
        ...
        if 'programming_language' not in comments:
            comments['programming_language'] = "R"

        comments['elements'] = comments['documentation'] + comments['comments']
        text = _insert_comments(text, comments['elements'], comments['programming_language'])
